...
export function HeroHeader({ userName, todaySales, monthRevenue, newLeads }: HeroHeaderProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl p-6 md:p-8 mb-6 animate-fade-in"
      style={{
        ...
        <div className="text-white">
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white text-foreground text-[13px] font-semibold hover:scale-105 transition-transform shadow-lg"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/15 backdrop-blur-sm border border-white/20 text-white text-[13px] font-semibold hover:bg-white/25 transition"
            >
...
function Stat({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div className="rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-3 hover:bg-white/15 transition">
      <div className="text-[10.5px] text-white/70 font-medium uppercase tracking-wide">{label}</div>
      <div className="text-white font-bold font-display text-lg md:text-xl mt-1 truncate" style={{ textShadow: "0 1px 8px oklch(0 0 0 / 0.2)" }}>
        {value}
      </div>
